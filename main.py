...
class MainProgram(framework.Framework):
    ...
    def add_drop_rows_bottom(self):
        ...
        # Get 2 first rows from main_array[:2]
        last_rows = self.main_array[-3:]
        for row in range(2):
            for column in range(len(last_rows[row])):
                last_rows[row][column] = 0
        ...
        if rows_after_click > self.rows_num:
            ...
            self.main_array = self.main_array + new_array
